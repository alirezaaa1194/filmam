...
async function Header({ absolute }: { absolute: boolean }) {
  ...
  return (
    <header className={`w-full max-w-layout-max h-fit lg:py-12 lg:px-24 mx-auto ${absolute ? "absolute inset-0 z-10" : ""}`}>
      <div className={`w-full h-12 lg:h-14 flex items-center justify-between gap-10 ${!absolute ? "bg-black " : ""} lg:bg-black/60 lg:rounded-lg lg:border lg:border-gray-12 lg:backdrop-blur-[15px] px-5 py-2 lg:p-3`}>
