  ...
  return (
    <Accordion key={menuItem.id} type="single" collapsible>
      <AccordionItem value={`${menuItem.id}`}>
        <AccordionTrigger showIcon={false} className="py-2 cursor-pointer w-full group">
        ...
        <AccordionContent className="px-3 pb-0">
          {menuItem.children.map((menuItemChild, j) => (
            <Fragment key={menuItemChild.id}>
              <Link href={menuItemChild.menu_type === MenuTypeEnum.PAGE ? `${menuItemChild.href}` : `/movies${menuItemChild.filter}`} className="block py-2">
                {menuItemChild.title}
              </Link>
