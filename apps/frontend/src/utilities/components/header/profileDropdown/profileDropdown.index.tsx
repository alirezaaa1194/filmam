      ...
      <DropdownMenu open={open} onOpenChange={setOpen} modal={false}>
        <DropdownMenuTrigger asChild>
          <Button
            variant="ghost"
            size="icon"
            className="!bg-complementary rounded-full border border-complementary-tint-2 w-[72] h-8 cursor-pointer flex items-center gap-1 !shadow-complementary-tint-2"
          >
            <UserOctagon
              className="size-5 fill-complementary-shade-5"
              variant="Bold"
            />
            <ArrowDown2
              className={`size-5 stroke-complementary-shade-5 transition-all ${open ? "rotate-180" : ""}`}
            />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent
          align="end"
          className="!mt-5 !p-3 w-[182px] bg-gray-13 border border-gray-12 rounded-lg flex flex-col gap-2"
        >
          <Link
            href="/"
            className="flex items-center justify-end gap-2 hover:bg-gray-12 transition-all w-full rounded-md"
            dir={dir === "rtl" ? "ltr" : "rtl"}
          >
          ...
          <Link
            ...
            dir={dir === "rtl" ? "ltr" : "rtl"}
          >
            <span className="text-white text-body-xxs">
              {t("Header.wishlist")}
            </span>
            <span className="flex items-center justify-center size-8 bg-gray-12 rounded-md">
          ...
          <button
            ...
            className="flex items-center justify-end gap-2 hover:bg-gray-12 transition-all w-full rounded-md cursor-pointer"
            dir={dir === "rtl" ? "ltr" : "rtl"}
          >
            <span className="text-complementary-tint-2 text-body-xxs">
              {t("Header.logout")}
            </span>
            <span className="flex items-center justify-center size-8 bg-gray-12 rounded-md">
