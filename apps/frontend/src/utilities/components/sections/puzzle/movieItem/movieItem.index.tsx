...
function PuzzleMovieItemComp({ movie }: { movie: MovieListItemType }) {
  ...
  return (
    <div className="h-max w-fit flex gap-3">
      <Image src={movieThumbnail?.path || ""} width={106} height={173} alt={movieThumbnail?.alt_text || movie.title} className="h-auto self-stretch xl:max-w-[105px] object-cover rounded-xl border border-gray-10 bg-gray-11" />
      <div className="hidden xl:flex flex-col justify-center gap-3">
        ...
        <span className="text-gray-8 flex items-center gap-1">
          ...
          <span className="text-body-xxs flex-1 text-nowrap line-clamp-1">{(movie.seasons_count ?? 0) > 1 ? `${movie.seasons_count} فصل` : `${movie.episodes_count} قسمت`}</span>
        </span>
      </div>
    </div>
  );
