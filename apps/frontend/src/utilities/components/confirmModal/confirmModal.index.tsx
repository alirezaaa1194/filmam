    ...
    >
      <ConfirmContent>
        <div className="flex flex-col items-center lg:px-6 lg:pb-12">
          ...
          <h5 className="text-mobile-h-5 lg:text-h-5 text-white lg:mt-6">
            {confirm?.title}
          </h5>
          <Separator className="bg-gray-10 mt-2 lg:hidden" />
          <p className="text-gray-7 lg:text-[#fff] text-body-xxs lg:text-button-md mt-2 lg:mt-8">
            {confirm?.description}
          </p>
          <div className="mt-4 lg:mt-12 flex gap-3 w-full">
            <Button
              className="flex-1 h-12 rounded-md cursor-pointer bg-primary border border-success text-body-xxs"
              onClick={handleConfirm}
              disabled={isPending}
            >
              {isPending || confirm?.isRefreshing ? <Spinner /> : null}{" "}
              {confirm?.submitText ?? "بله"}
            </Button>
            <Button
              onClick={() => {
                setConfirm(null);
