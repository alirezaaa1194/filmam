...
function MovieHeaderComp({ movie }: { movie: MovieDetailPublicType }) {
  ...
  return (
    <section className="w-full relative lg:h-[624px] 2xl:h-screen">
      <Image src={bannerUrl?.path || placeholderPath} alt={bannerUrl?.alt_text || movie.title} className="w-full h-[300px] lg:h-full object-cover object-top bg-gray-11" width={800} height={240} />
      <div className="absolute z-10 top-0 left-0 w-full h-[300px] lg:h-full bg-[linear-gradient(0deg,rgba(12,12,12,1)_0%,rgba(12,12,12,0.96)_13%,rgba(12,12,12,0.4)_33%,rgba(12,12,12,0)_47%,rgba(12,12,12,0.28)_68%,rgba(12,12,12,1)_100%)]" />
      ...
      <div className="lg:absolute z-20 top-0 left-0 right-0 w-full max-w-layout-max mx-auto flex gap-12 lg:h-full px-layout-x-space pb-10 xl:pb-5">
        ...
        <div className="w-full min-w-0 h-full flex flex-col justify-end gap-2 lg:gap-8 items-start lg:py-6">
          <div className="flex flex-col gap-2 w-full">
            ...
            <div className="w-full min-w-0 flex justify-between lg:justify-start gap-4 lg:gap-10 text-nowrap overflow-x-auto lg:scrollbar-none text-caption-md lg:text-body-xxs my-3">
              {movieDirector ? (
