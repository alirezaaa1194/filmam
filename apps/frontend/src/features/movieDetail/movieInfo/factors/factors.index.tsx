        ...
        <div className="flex flex-wrap gap-2 lg:gap-5">
          {actorFactors?.map((factor) => (
            <div key={`actor-${factor.id}`} className="flex items-center gap-2 p-1 border border-gray-11 rounded-full pe-3">
              <Image src={factor?.profile?.path || userPlaceholder} width={48} height={48} alt={`${factor.first_name} ${factor.last_name}`} className="size-12 rounded-full bg-gray-11 border border-gray-11 shrink-0 object-cover" />
