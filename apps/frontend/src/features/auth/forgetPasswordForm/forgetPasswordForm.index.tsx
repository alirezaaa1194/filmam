...
function ForgetPasswordForm() {
  ...
  return (
    <div className="w-full p-6">
      ...
      {step === "Email" ? <ForgetEmailForm setStep={setStep} start={start} defaultValues={{ email }} onSubmit={(value) => setEmail(value.email)} /> : <ForgetOtpForm setStep={setStep} start={start} reset={reset} timer={timer} email={email} />}
    </div>
  );
}
