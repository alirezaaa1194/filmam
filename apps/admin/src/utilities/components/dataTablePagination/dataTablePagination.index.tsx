...
import { Cn, GetPageNumbers } from '@/scripts'
import { Button, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/utilities/components'
    ...
    >
      <div className='flex w-full items-center justify-between'>
        ...
        <div className='flex items-center gap-2 @max-2xl/content:flex-row-reverse'>
          ...
          <p className='hidden text-sm font-medium sm:block'>{t('common.rows_per_page')}</p>
        </div>
      </div>
          ...
          {pageNumbers.map((pageNumber, index) => (
            <div key={`${pageNumber}-${index}`} className='flex items-center'>
              {pageNumber === '...' ? (
                ...
                >
                  <span className='sr-only'>{t('common.go_to_page', { page: pageNumber })}</span>
                  {pageNumber}
                </Button>
              )}
            </div>
