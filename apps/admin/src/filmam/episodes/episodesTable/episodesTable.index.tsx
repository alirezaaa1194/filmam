...
import { episodesColumns as columns } from '../episodesColumns/episodesColumns.index'
import { EpisodesTableSkeleton } from '../episodesTableSkeleton/episodesTableSkeleton.index'

const route = getRouteApi('/_authenticated/episodes/')

type DataTableProps = { data: Episode[]; count: number; isPending: boolean }

export function EpisodesTable({ data, count, isPending }: DataTableProps) {
    ...
    >
      <div role='toolbar' className='flex items-center justify-between'>
        <div className='flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:space-x-2'>
          <Input
            placeholder={t('episodes.filter_placeholder')}
            value={searchInput}
        ...
        <DataTableViewOptions
          ...
          labels={{
            ...
            slug: t('episodes.slug'),
            order: t('episodes.order'),
            season_title: t('episodes.season_title'),
            movie_title: t('episodes.movie_title'),
            likes_count: t('episodes.likes_count'),
            dislikes_count: t('episodes.dislikes_count'),
            watches_count: t('episodes.watches_count'),
            created_at: t('episodes.created_at'),
          }}
        />
      </div>
      {isPending ? (
