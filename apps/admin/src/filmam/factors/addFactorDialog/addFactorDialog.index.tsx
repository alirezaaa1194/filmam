...
import type { CreateFactorPayloadType } from '../factors.type'

type FactorAddDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function AddFactorDialog({ open, onOpenChange }: FactorAddDialogProps) {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (data: FactorForm) =>
      Api(AppApis.factor.adminCreate, {
        method: 'POST',
        body: {
          slug: data.slug,
    ...
    onSuccess: () => {
      toast.success(t('factors.factor_created'))
      form.reset()
      setProfileFiles([])
      onOpenChange(false)
    ...
    >
      <DialogContent className='sm:max-w-lg'>
        <DialogHeader className='text-start'>
          <DialogTitle>{t('factors.add_factor')}</DialogTitle>
          <DialogDescription>{t('factors.add_factor_desc')}</DialogDescription>
        </DialogHeader>
        <div className='w-[calc(100%+0.75rem)] overflow-y-auto py-1 pe-3'>
          <Form {...form}>
            ...
            >
              ...
              <Tabs
                defaultValue={factorTabLanguageOrder[0]}
              ...
              <DialogFooter>
                <Button type='submit' form='factor-form' disabled={isPending}>
