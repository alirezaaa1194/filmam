...
import { RoleDialogSkeleton } from './roleDialogSkeleton.index'
...
export function EditRoleDialog({
  ...
  const {
    ...
    queryKey: ['roles-detail', currentRow.id],
    queryFn: () =>
      Api<RoleDetailType>(AppApis.role.adminById(currentRow.id), {
        method: 'GET',
      }),
    enabled: open,
    staleTime: 0,
  ...
  const isLoading = isPending || isFetching || !roleDetail
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='sm:max-w-lg'>
        <DialogHeader className='text-start'>
          ...
          <DialogDescription>{t('roles.edit_role_desc')}</DialogDescription>
        </DialogHeader>
        <div className='w-[calc(100%+0.75rem)] overflow-y-auto py-1 pe-3'>
          {isLoading ? (
            <RoleDialogSkeleton />
          ) : (
            <RoleEditForm roleDetail={roleDetail} onOpenChange={onOpenChange} />
          )}
        </div>
      </DialogContent>
    </Dialog>
...
function RoleEditForm({
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (data: RoleForm) =>
      Api(AppApis.role.adminUpdate(roleDetail.id), {
        method: 'PUT',
        body: {
          slug: data.slug,
          type: data.type,
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        <Tabs defaultValue={roleTabLanguageOrder[0]} className='col-span-6'>
        ...
        <DialogFooter>
          <Button type='submit' form='role-edit-form' disabled={isPending}>
