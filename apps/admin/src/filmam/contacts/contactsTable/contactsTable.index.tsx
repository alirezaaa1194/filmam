...
import { contactStatuses, type Contact } from '../contacts.type'
import { DataTableBulkActions } from '../dataTableBulkActions/dataTableBulkActions.index'
import { contactsColumns as columns } from '../contactsColumns/contactsColumns.index'
import { ContactsTableSkeleton } from '../contactsTableSkeleton/contactsTableSkeleton.index'

const route = getRouteApi('/_authenticated/contacts/')

type DataTableProps = { data: Contact[]; count: number; isPending: boolean }

export function ContactsTable({ data, count, isPending }: DataTableProps) {
      ...
      <div role='toolbar' className='flex items-center justify-between'>
          ...
          <div className='flex flex-wrap gap-x-2'>
            ...
            >
              <SelectTrigger className='h-8 w-fit gap-1'>
                <SelectValue placeholder={t('contacts.all_statuses')} />
              </SelectTrigger>
        ...
        <DataTableViewOptions
          ...
          labels={{
            ...
            is_registered: t('contacts.is_registered'),
            status: t('contacts.status'),
            created_at: t('contacts.created_at'),
          }}
        />
      </div>
      {isPending ? (
