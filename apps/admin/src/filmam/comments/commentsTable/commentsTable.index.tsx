...
import { commentStatuses, type Comment } from '../comments.type'
import { DataTableBulkActions } from '../dataTableBulkActions/dataTableBulkActions.index'
import { commentsColumns as columns } from '../commentsColumns/commentsColumns.index'
import { CommentsTableSkeleton } from '../commentsTableSkeleton/commentsTableSkeleton.index'

const route = getRouteApi('/_authenticated/comments/')

type DataTableProps = { data: Comment[]; count: number; isPending: boolean }

export function CommentsTable({ data, count, isPending }: DataTableProps) {
    ...
    >
      <div role='toolbar' className='flex items-center justify-between'>
        <div className='flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:space-x-2'>
          <Input
            placeholder={t('comments.filter_placeholder')}
            value={searchInput}
          ...
          <div className='flex flex-wrap gap-x-2'>
                ...
                navigate({
                  search: (prev) => ({
                    ...
                    status:
                      value === ''
                        ? undefined
                        : (value as NonNullable<typeof prev.status>),
                    page: undefined,
                  }),
                })
              }}
            >
              <SelectTrigger className='h-8 w-fit gap-1'>
                <SelectValue placeholder={t('comments.all_statuses')} />
              </SelectTrigger>
              <SelectContent side='bottom'>
                <SelectItem value=''>
                  {t('comments.all_statuses')}
                </SelectItem>
                {commentStatuses.map((status) => (
                  <SelectItem key={status} value={status}>
                    {t(`comments.statuses.${status}`)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        <DataTableViewOptions
          ...
          labels={{
            ...
            entity: t('comments.entity'),
            status: t('comments.status'),
            likes_count: t('comments.likes_count'),
            dislikes_count: t('comments.dislikes_count'),
