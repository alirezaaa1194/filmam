...
import { Api, TranslateServerError, TimerParser, Cn } from '@/scripts'
import {
  Button,
  Card,
  CardContent,
...
export function OtpForm({
  ...
  const { mutate: forgetPasswordMutate, isPending: forgetPasswordIsPending } =
    useMutation({
  ...
  const { mutate: resetPasswordMutate, isPending: resetPasswordIsPending } =
    useMutation({
      mutationFn: ({
        email,
        otp,
        ...
        Api(AppApis.auth.adminResetPassword, {
          method: 'PUT',
          body: {
            email,
  ...
  function onSubmit(data: z.infer<typeof formSchema>) {
    resetPasswordMutate({ ...data, email })
  }
          ...
          <div className='my-4 flex w-full items-center justify-between'>
            ...
            {timer > 0 ? (
              ...
              <Button
                ...
                size='sm'
                type='button'
                onClick={() => forgetPasswordMutate({ email })}
      ...
      <CardContent>
        <Form {...form}>
          ...
          >
            ...
            <Button
              className='mt-2'
              disabled={otp.length < 5 || resetPasswordIsPending}
            >
              {resetPasswordIsPending ? <Spinner /> : null}
              {t('auth.verify')}
            </Button>
          </form>
        </Form>
