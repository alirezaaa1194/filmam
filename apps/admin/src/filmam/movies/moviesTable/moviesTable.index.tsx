...
import { moviesColumns as columns } from '../moviesColumns/moviesColumns.index'
import { MoviesTableSkeleton } from '../moviesTableSkeleton/moviesTableSkeleton.index'
...
export function MoviesTable({ data, count, isPending }: DataTableProps) {
  ...
  const isFiltered =
    ...
    (search.sortBy ?? 'CREATED_AT') !== 'CREATED_AT' ||
    !!search.type
    ...
    >
      <div role='toolbar' className='flex items-center justify-between'>
        <div className='flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:space-x-2'>
          ...
          <div className='flex gap-x-2'>
            <Select
              ...
              onValueChange={(value) => {
                const type =
                  value === 'CINEMATIC' || value === 'SERIES' ? value : undefined
                navigate({
                  search: (prev) => ({
                    ...prev,
                    type,
            ...
            >
              ...
              <SelectContent side='bottom'>
                <SelectItem value='ALL'>{t('movies.all_types')}</SelectItem>
                <SelectItem value='CINEMATIC'>
                  {t('movies.type_cinematic')}
                </SelectItem>
                <SelectItem value='SERIES'>{t('movies.type_series')}</SelectItem>
              </SelectContent>
            </Select>
            <Select
            ...
            >
              <SelectTrigger className='h-8 w-fit gap-1'>
                <SelectValue placeholder={t('movies.sort_by')} />
              </SelectTrigger>
              <SelectContent side='bottom'>
                <SelectItem value='CREATED_AT'>
                  {t('movies.sort_created_at')}
                </SelectItem>
                <SelectItem value='LIKES'>{t('movies.sort_likes')}</SelectItem>
                <SelectItem value='WATCHES'>{t('movies.sort_watches')}</SelectItem>
              </SelectContent>
            </Select>
            <Select
        ...
        <DataTableViewOptions
          ...
          labels={{
            title: t('movies.title_column'),
            slug: t('movies.slug'),
            type: t('movies.type'),
            released_year: t('movies.released_year'),
            genres: t('movies.genres'),
            created_at: t('movies.created_at'),
            age_limit: t('movies.age_limit'),
            likes_count: t('movies.likes_count'),
            dislikes_count: t('movies.dislikes_count'),
          }}
        />
      </div>
      {isPending ? (
