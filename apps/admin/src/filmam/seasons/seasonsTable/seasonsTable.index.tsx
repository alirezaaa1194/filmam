...
import { seasonsColumns as columns } from '../seasonsColumns/seasonsColumns.index'
import { SeasonsTableSkeleton } from '../seasonsTableSkeleton/seasonsTableSkeleton.index'

const route = getRouteApi('/_authenticated/seasons/')

type DataTableProps = { data: Season[]; count: number; isPending: boolean }

export function SeasonsTable({ data, count, isPending }: DataTableProps) {
    ...
    >
      <div role='toolbar' className='flex items-center justify-between'>
        <div className='flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:space-x-2'>
          <Input
            placeholder={t('seasons.filter_placeholder')}
            value={searchInput}
        ...
        <DataTableViewOptions
          table={table}
          columns={['order', 'movie_title', 'episodes_count', 'created_at']}
          labels={{
            title: t('seasons.title'),
            slug: t('seasons.slug'),
            order: t('seasons.order'),
            movie_title: t('seasons.movie_title'),
            episodes_count: t('seasons.episodes_count'),
            created_at: t('seasons.created_at'),
          }}
        />
      </div>
      {isPending ? (
        <SeasonsTableSkeleton />
      ) : (
